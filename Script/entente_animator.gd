extends Node
# Model-swap animator named "AnimationPlayer" so player.gd's find_child() picks
# it up: one FBX per state as a child, visibility toggled on play().
...
const MODEL_MAP: Dictionary = {
	&"offensive idle":    BASE + "Offensive Idle.fbx",
...
func _setup_models() -> void:
	for anim_name: StringName in MODEL_MAP:
		...
		if not scene:
			push_warning("EntenteAnimator: cannot load %s" % MODEL_MAP[anim_name])
			continue
...
# static "Take 001" that sorts first. Prefer the real one or the model T-poses.
